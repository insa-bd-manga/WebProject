...
def actus(request, num_page=0):
    ...
    n = 10
    query = Article.objects.all().order_by("-date")[n*(num_page):n*(num_page+1)]
...
def archives(request, tag="", year=timezone.now().year, month=timezone.now().month, num_page=0):
    ...
    n = 10
    query = Article.objects.filter(date__year=year).filter(date__month=month).filter(tag__nom_tag__contains=tag).order_by("-date")[n * (num_page):n * (num_page + 1)]
    return render(request, 'vitrine/actus.html', {"last_articles": query, "page": num_page})
